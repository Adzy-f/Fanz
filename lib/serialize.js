...
// ini bakal ambil config.json dari root project (~/Fanz/config.json)
   ...
   const client = Object.defineProperties(conn, {
      getJid: {
         value(sender) {
            ...
            for (let chat of Object.values(conn.chats)) {
               ...
               let user = chat.participants.find(p => p.lid === sender || p.id === sender);
               if (user) {
      ...
      sendSticker: {
         async value(jid, path, m, options = {}) {
            ...
            let exif = {
               packName: options.pack || cfg.pck.name,
               packPublish: options.author || cfg.pck.author
            }
            let sticker = await (await import('./exif.js')).writeExif({
               mimetype: mime,
               data
            }, exif);
...
export default async function serialize(conn, msg) {
   ...
   if (msg.key) {
      ...
      m.chat = conn.getJid(jidNormalizedUser(m.key.remoteJidAlt || m.key.remoteJid));
   ...
   if (m.message) {
      ...
      if (m.msg?.contextInfo?.quotedMessage) {
         ...
         if (m.quoted.message) {
            ...
            m.quoted.sender = conn.getJid(jidNormalizedUser(m.msg?.contextInfo?.participant || m.quoted.from))
            let mention = [...(m.quoted.msg?.contextInfo?.mentionedJid || []), ...(m.quoted.msg?.contextInfo?.groupMentions?.map(v => v.groupJid) || [])]
            m.quoted.mentions = mention.map(jid => conn.getJid(jid))
         ...
         return conn.sendMessage(m.chat, {
            ...
            contextInfo: {
               ...
               externalAdReply: {
                  title: cfg.set.botN,
                  body: cfg.set.wm,
                  thumbnailUrl: cfg.set.thumb,
                  sourceUrl: cfg.set.url,
