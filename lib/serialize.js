   ...
   const client = Object.defineProperties(conn, {
      ...
      sendSticker: {
         async value(jid, path, m, options = {}) {
            ...
            let exif = {
               packName: options.pack || cfg.pack,
               packPublish: options.author || cfg.auth
            }
...
export default async function serialize(conn, msg) {
   ...
   if (msg.key) {
      ...
      m.chat = conn.getJid(m.key.remoteJidAlt || m.key.remoteJid);
   ...
   m.vcard = (name, vcard) => => conn.sendMessage(m.chat, {
      contacts: {
         displayName: name,
         contacts: [{
            vcard
