   ...
   const client = Object.defineProperties(conn, {
      ...
      sendSticker: {
         async value(jid, path, m, options = {}) {
            ...
            let exif = {
               packName: options.pack || c.pack,
               packPublish: options.author || c.auth
            }
...
export default async function serialize(conn, msg) {
   ...
   // Quick reply
   m.ads = async (txt, title, body, url, img, size = false) => {
      return conn.sendMessage(m.chat, {
         ...
         contextInfo: {
            ...
            externalAdReply: {
               title: title || c.botF,
               body: body || c.wm,
               thumbnailUrl: img || c.thumb,
               sourceUrl: url || c.url,
               renderLargerThumbnail: size
            }
         }
      }, {
         quoted: m,
